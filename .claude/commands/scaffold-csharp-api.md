...
> **Naming warning:** Do NOT use `Task` as a domain class name — it conflicts with
> `System.Threading.Tasks.Task` and causes ambiguity errors throughout the codebase.
> The scaffold uses `WorkItem` as the sample domain class to avoid this collision.
> When adapting the scaffold for your own domain, choose names that do not shadow BCL types.
...
4. **Configure Program.cs** — Wire the full middleware pipeline:
   ...
   - Register `IWorkItemRepository` → `DapperWorkItemRepository` and `IWorkItemService` → `WorkItemService` as `Scoped`
...
8. **Create domain model** — Define a sample `WorkItem` domain:
   - `readonly record struct WorkItemId(Guid Value)` with `New()`, `From(Guid)`, and `TryParse(string)` factory methods
   - `sealed class WorkItem` with private constructor, `[JsonConstructor]` attribute, `Create(string title)` factory, and `Reconstitute(...)` for persistence mapping
   - Abstract record `WorkItemError` hierarchy: `NotFound(WorkItemId Id)`, `ValidationError(string Message)`, `Conflict(string Message)`
   - `Result<T>` type with `Success(T)` / `Failure(WorkItemError)` statics and `Match<TOut>` method
   Read `reference/csharp-dotnet-templates.md` for templates.

9. **Create repository layer** — `IWorkItemRepository` interface with `async Task<T>` methods and `CancellationToken` on every signature. Implement `DapperWorkItemRepository` using `SqlConnection`, parameterised Dapper queries, and a private `sealed record` row DTO for mapping. Never expose the row DTO outside the repository class. Read `reference/csharp-dotnet-templates.md` for template.

10. **Create service layer** — `IWorkItemService` interface returning `Task<Result<T>>`. Implement `WorkItemService` with `IWorkItemRepository` and `ILogger<WorkItemService>` injected via primary constructor. All domain errors returned as `Result.Failure(...)` — never throw for domain outcomes. Pass `CancellationToken` through to every repository call. Read `reference/csharp-dotnet-templates.md` for template.

11. **Create controller** — `WorkItemsController` with `[ApiController]`, `[Route("api/v1/workitems")]`, `[Produces("application/json")]`. All actions `async Task<IActionResult>` with `CancellationToken ct` parameter. Map `Result<T>` to HTTP via `switch` on `WorkItemError` — exhaustive, no wildcard catch-all except a logged 500 fallback. Use `CreatedAtAction` for POST. Read `reference/csharp-dotnet-templates.md` for template.

12. **Create validator** — `CreateWorkItemRequestValidator : AbstractValidator<CreateWorkItemRequest>` with `RuleFor(x => x.Title).NotEmpty().MaximumLength(200)`. Registered automatically via `AddValidatorsFromAssemblyContaining<Program>()` — no manual wiring needed.
...
16. **Write tests** — Unit tests in `$ARGUMENTS.Tests/Services/WorkItemServiceTests.cs` using xUnit + NSubstitute + FluentAssertions with `NullLogger<WorkItemService>.Instance`. Cover:
    - `ListAllAsync` returns repository result
    - `GetByIdAsync` returns `Success` when work item exists
    - `GetByIdAsync` returns `NotFound` error when work item missing
    - `CreateAsync` with valid title returns created work item and calls `SaveAsync`
    ...
    Add integration test in `$ARGUMENTS.Tests/Controllers/WorkItemsControllerIntegrationTests.cs` using `ApiFactory` (custom `WebApplicationFactory<Program>` with `InMemoryWorkItemRepository` replacing `DapperWorkItemRepository`). Read `reference/csharp-dotnet-templates.md` for templates.
...
19. **Print summary** — List all created files, `dotnet run` to start (port 5000 HTTP / 5001 HTTPS), Swagger UI at `/swagger`, health check at `/api/v1/health`, next steps (swap `DapperWorkItemRepository` for real DB, add auth middleware, add EF Core migrations, etc.).
